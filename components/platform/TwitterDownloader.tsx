...
import { FaTwitter, FaDownload, FaMusic } from 'react-icons/fa';
...
export default function TwitterDownloader() {
  ...
  const handleDownload = async (format: PostFormat, type: string = 'video') => {
    try {
      ...
      const filename = filenameMatch ? filenameMatch[1] : `twitter-${type}-${format.quality}.${format.container}`;
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div className="bg-gradient-to-r from-blue-400 to-blue-500 p-6 rounded-xl shadow-xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            <FaTwitter className="absolute left-4 top-1/2 transform -translate-y-1/2 text-white text-xl" />
            <input
              type="url"
              value={url}
              onChange={(e) => setUrl(e.target.value)}
              placeholder="Paste Twitter/X post link here..."
              className="w-full pl-12 pr-4 py-4 rounded-lg bg-white/10 text-white placeholder-gray-300 focus:ring-2 focus:ring-blue-300 focus:outline-none backdrop-blur-sm"
              required
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-white text-blue-500 font-bold py-4 px-6 rounded-lg 
              disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300
              hover:bg-blue-50 transform hover:scale-[1.02]"
          >
